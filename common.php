...
//';
    ...
    if (isset($_GET['AddDisk'])) {
        if ($_SERVER['admin']) {
                require_once 'disk' . $slash . $_GET['AddDisk'] . '.php';
    ...
    require_once 'disk' . $slash . $disktype . '.php';
    ...
    if ($_SERVER['ajax']) {
        ...
        if ($_GET['action']=='upbigfile') return $drive->bigfileupload($path);
    }
    if ($_SERVER['admin']) {
    ...
    if ($_SERVER['is_guestup_path'] && !$_SERVER['admin']) {
        ...
        else $files = $drive->list_files($path);
    } else {
