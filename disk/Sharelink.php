...
class Sharelink extends Onedrive {
    ...
    function __construct($tag) {
        ...
        $this->api_url = getConfig('shareapiurl', $this->disktag);
        $this->sharecookie = getConfig('sharecookie', $this->disktag);
        $this->DownurlStrName = '@content.downloadUrl';
    }

    public function isfine()
    {
        //error_log('A_T:' . $this->access_token);
        if (!getConfig('shareurl', $this->disktag)) return false;
        else return true;
    }
    ...
    protected function get_access_token($refresh_token) {
        if (!($this->access_token = getcache('access_token', $this->disktag))) {
            ...
            if (!($this->sharecookie = getConfig('sharecookie', $this->disktag))) {
