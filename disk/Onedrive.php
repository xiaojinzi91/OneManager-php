...
class Onedrive {
            ...
            if ($arr['stat']<500) {
                ...
                if (isset($files['folder'])) {
                    if ($files['folder']['childCount']>200) {
                        ...
                        if ($page>1) $files = $this->fetch_files_children($files, $path1, $page);
    ...
    protected function fetch_files_children($files, $path, $page)
    {
        ...
        if (!($files['children'] = getcache('files_' . $path . '_page_' . $page, $this->disktag))) {
            ...
            $cachefile = $this->list_files(path_format($path1 . '/' .$cachefilename));
            if ($cachefile['size']>0) {
...
<div>
    <form action="?install4&AddDisk=' . get_class($this) . '" method="post" onsubmit="return notnull(this);">
        ...
        <label>
            ...
            <div style="width:100%;margin:0px 35px"><a href="' . $Tenant . '/_layouts/15/sharepoint.aspx" target="_blank">Click to find a site or create a new site.</a><br>' . getconstStr('GetSharepointSiteAddress') . '<br>
...
<div>
    ...
    public function bigfileupload($path)
    {
        ...
        if ($_GET['upbigfilename']!=''&&$_GET['filesize']>0) {
            ...
            $response = $this->MSAPI('createUploadSession', path_format($path1 . '/' . $filename), '{"item": { "@microsoft.graph.conflictBehavior": "fail" }}', $this->access_token);
            if ($response['stat']<500) {
                $responsearry = json_decode($response['body'],true);
                if (isset($responsearry['error'])) return output($response['body'], $response['stat']);
                $fileinfo['uploadUrl'] = $responsearry['uploadUrl'];
                if ($fileinfo['size']>10*1024*1024) $this->MSAPI('PUT', path_format($path1 . '/' . $cachefilename), json_encode($fileinfo, JSON_PRETTY_PRINT), $this->access_token);
            }
